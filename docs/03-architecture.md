...
| 버전 | 1.2 |
| 작성일 | 2026-09-02 |
| 상태 | 구현 반영 (2026-09-26). 구현하면서 바뀐 곳은 이 문서에 고쳐 두었고, 결정의 근거는 각 ADR에 있다 |
...
flowchart TB
    ...
    subgraph obs["관측"]
        cwl["CloudWatch Logs<br/>14일 보존"]
    end

    subgraph audit["상시 환경 envs/audit"]
        trail["CloudTrail"]
        s3["S3 Object Lock<br/>규정 준수 1일"]
    end
...
| ⑤ | 로그 | WAF, 검사 계층, 흐름 로그 → CloudWatch Logs. API 호출은 CloudTrail → 잠금 버킷(`envs/audit`). 앱은 로그를 보내지 않는다 |
...
| `FR-08` 공격 재현·검증 | 측정 도구 `tools/probe`(`ADR-020`), 배포 검증 `tools/verify`(`ADR-036`) |
...
> **배포로 확인했다 (Phase 8, 9).** 경로를 켠 뒤 관리형 방화벽의 흐름 로그에 요청과 응답이
> 양방향 모두 기록됐고, 같은 공격이 평문 80에서는 검사 계층에서, 443에서는 WAF에서 막혔다.
> 방식 B에서도 같은 경로로 동작했다. 경로를 켜는 값과 순서는 `ADR-024`, 방식별 목적지는 `ADR-026`.
> 경로를 켠 직후 수십 초 동안은 엣지 경로가 반영되지 않아 평문 요청이 검사 계층을 거치지 않는다(`ADR-036`).
...
| 방향 | 대상 | 포트 | 근거 |
|---|---|---|---|
| 인바운드 | 운영자 주소 `/32` | 443 | `FR-01`. 배포할 때 조회한 주소만 허용 (`ADR-019`, `T-44`) |
| 인바운드 | 운영자 주소 `/32` | 80 | `T-11` 계층별 탐지 범위 검증용 (의도적 유지) |
...
| 방향 | 대상 | 포트 | 근거 |
|---|---|---|---|
| 인바운드 | 운영자 주소 | 80, 443 | 통과시킬 인바운드 트래픽. 웹 방화벽이 받는 출발지와 같게 한정 (`ADR-019`, `ADR-027`) |
| 인바운드 | `10.20.0.0/24` | 443 | 웹 방화벽 자신의 아웃바운드 (패키지, 세션 관리자) |
...
| 역할 | 허용 작업 | 대상 리소스 |
|---|---|---|
| `waf-role` | 로그 스트림 생성, 기록 | WAF 전용 로그 그룹 |
| | 내부 CA 인증서 **읽기** (`GetSecretValue`) | CA 인증서 시크릿 하나 |
| | 세션 관리 서비스 연결 (액션 5개) | `*` — 리소스 단위 권한을 지원하지 않는 액션 |
| `app-role` | 내부 CA 인증서 **쓰기** (`PutSecretValue`) | CA 인증서 시크릿 하나 |
| | 세션 관리 서비스 연결 (액션 5개) | `*` |
| `suricata-role` | 로그 스트림 생성, 기록 | Suricata 전용 로그 그룹 |
| | 세션 관리 서비스 연결 (액션 5개) | `*` |
| `vpc-flow-role` | 로그 스트림 생성, 기록, 조회 | 흐름 로그 그룹 (흐름 로그 서비스가 맡는 역할) |

- 세 EC2 역할과 흐름 로그 역할의 신뢰 정책에는 `aws:SourceAccount` 조건을 둔다. 다른 계정의 요청으로 서비스가 이 역할을 맡는 경로(혼동된 대리인)를 막는다(`ADR-036`).
- 세션 관리 권한은 관리형 정책 대신 필요한 액션 5개만 준다. 이것만으로 세션 접속과 원격 실행이 동작하는 것을 Phase 6에서 확인했다.
- 앱은 CA 인증서를 쓰기만, WAF는 읽기만 한다. 신뢰의 기준을 바꿀 수 있는 주체가 앱 하나로 한정된다(`ADR-017`).
- 앱은 로그 에이전트가 없어 로그 권한을 두지 않는다(`SR-16`).
...
| 소스 | 수집 경로 | 로그 그룹 | 근거 |
|---|---|---|---|
| 방화벽 (방식 A) | 서비스 네이티브 | `/soc-lab/firewall/alert`, `/soc-lab/firewall/flow` | `SR-15` |
...
```
bootstrap/            상태 버킷 (로컬 상태, 한 번만)
envs/
  lab/                랩 진입점. 필요할 때 만들고 지운다
  audit/              상시 환경: CloudTrail, 잠금 버킷, 계정 기본 설정, 예산
modules/
  network/            VPC, 서브넷, 라우팅(경로 전환 포함), IGW, NAT, 보안 그룹
  web/                앱 인스턴스, 역할, 내부 CA와 서버 인증서 생성(부팅 중)
  secrets/            CA 인증서를 전달하는 시크릿 (값은 Terraform이 쓰지 않음)
  waf/                WAF 인스턴스, ModSecurity, CRS, 역할, 로그 그룹
  firewall_managed/   관리형 방화벽, 룰그룹, 정책, 경보와 흐름 로그 그룹
  firewall_oss/       Suricata 인스턴스, nftables 큐, 역할, 로그 그룹
  observability/      VPC 흐름 로그, 지표 필터, 경보, 저장 쿼리, 대시보드
rules/                두 방식이 같이 쓰는 Suricata 룰
scripts/              로그 에이전트 설치, 시간 동기화 (부팅 스크립트에 값으로 삽입)
```

사설 CA는 처음 설계에서 `waf` 모듈에 두려 했으나, 개인 키를 Terraform 상태에 남기지 않으려고 앱 인스턴스가 부팅 중에 만들고 공개 인증서만 시크릿으로 넘기는 구조로 바꿨다(`ADR-017`).

| 변수 | 값 | 효과 |
|---|---|---|
| `firewall_mode` | `managed` / `oss` | 배포할 검사 계층과 경로 목적지의 종류 (`ADR-026`) |
| `route_through_firewall` | `true` / `false` | 경로를 검사 계층으로 넘길지. 검사 계층을 먼저 띄우고 접속을 확인한 뒤 켠다 (`ADR-024`) |
| `az_count` | 정수 | 배포할 가용영역 수 (`NFR-05`, network 모듈 변수. 랩은 1) |
...
`firewall_mode` 변수로 배포할 방식과 라우팅 타깃의 종류가 정해지고, 경로를 실제로 넘기는 것은 `route_through_firewall`로 따로 적용한다(`ADR-024`, `ADR-026`).
...
| 관제 | 로그를 SIEM(또는 정규화 저장소)으로 넘겨 상관 분석과 경보 운영 | 비용과 범위. 이 환경의 CloudWatch는 **수집 지점과 조사 도구**다(`ADR-030`) |

> **관제 구성에 대하여.** 실무 보안관제의 중심은 SIEM이고, CloudWatch와 CloudTrail, VPC 흐름 로그는
> 그 앞단의 원천이다. 이 환경이 Phase 10에서 만든 것(에이전트 설치, 수집 선별, 보내기 전 마스킹,
> 두 방식의 로그 형식 통합)은 SIEM을 붙이더라도 그대로 필요한 앞 절반이다. 형식 통합은 SIEM의 파싱과
> 정규화, AWS Security Lake의 OCSF 변환이 푸는 문제와 같다.
...
| `app-sg` 아웃바운드 80 | 앱의 패키지 저장소 접근용으로 열어 둠 | ⚠️ 유지 중. Ubuntu인 WAF와 Suricata는 저장소를 HTTPS로 바꿔 80을 쓰지 않는다(Phase 7). Amazon Linux 2023인 앱이 80 없이 동작하는지는 **확인하지 않았다** |
...
| 항목 | 내용 | 확정 시점 |
|---|---|---|
| ~~라우팅 동작~~ | ✅ **해소 (Phase 8, 9).** 흐름 로그에 양방향 기록, 80과 443의 계층별 차단 확인 (3.2절) | Phase 8 |
...
| ~~Suricata 포워딩~~ | ✅ **해소 (Phase 9).** 단일 ENI, forward 훅만 큐. fail-close 시험 완료. 부팅 순서 결함을 고쳐 부팅 중과 부팅 실패 시에도 막히도록 함(`ADR-027`, 2026-09-26) | Phase 9 |
| ~~ModSecurity 패키지~~ | ✅ **해소 (2026-09-05).** Ubuntu 24.04 universe에 패키지 전부 존재(`ADR-016`) | Phase 7 |
| 패키지 저장소 프로토콜 | WAF와 Suricata는 HTTPS로 전환. 앱의 80은 확인하지 않고 유지 (5.2절) | 미정 |
| `T-04` 파싱 불일치 | ⚠️ **미검증.** 별도 통제를 구성하지 않았고 파라미터 중복, 요청 스머글링을 시험하지 않았다. nginx가 요청을 다시 만들어 넘기고 ModSecurity가 같은 이름의 인자를 모두 검사하므로 완화될 것으로 보이나 추정이다 | 미정 |

---

## 7. 추적성 매트릭스 — 통제

설계 시점의 통제 배치다. 각 요구사항이 실제로 충족됐는지의 최종 판정은 `05-verification.md`에 있다.
...
| `SR-25` | `T-38` | 프로바이더, AMI(이름과 소유자), 앱 이미지(다이제스트), CI 액션(커밋 SHA), 에이전트(서명) | Ph.4~11 | `AC-08` |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| 1.2 | 2026-09-26 | 구현 결과 반영 — 구성도와 로그 흐름(S3 이관 없음, 앱 로그 없음, 감사 로그는 `envs/audit`), WAF 인바운드를 운영자 주소로(`ADR-019`), IAM 표에 시크릿 권한과 신뢰 정책 조건 추가, 모듈 구조와 변수, 미확인 항목 해소 여부. `T-04`와 앱의 80번 아웃바운드는 미검증으로 명시 |
