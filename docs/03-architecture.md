...
| `app-role` | CA 인증서 값 쓰기 | CA 인증서 시크릿 (로그 에이전트가 없어 로그 권한은 두지 않는다) |
| | 세션 관리 서비스 연결 | 해당 인스턴스 |
...
modules/
  ...
  firewall_managed/   관리형 방화벽 · 룰그룹 · 정책 · 엔드포인트
  firewall_oss/       Suricata 인스턴스 · nftables · 룰셋
...
| Suricata 포워딩 | 단일 ENI, forward 훅만 큐로 보내는 구성으로 확정(`ADR-027`). 실물 동작은 배포로 확인 | Phase 9 |
