...
| 버전 | 1.4 |
| 작성일 | 2026-09-01 |
| 상태 | 전 경계(A~E) 작성 완료. 통제의 구현과 검증 결과는 `05-verification.md` |
| 방법론 | STRIDE |
| 관련 문서 | `01-requirements.md`, `03-architecture.md`, `05-verification.md` |
...
| `T-44` | E 상승 | 의도적으로 취약한 검증 대상의 인터넷 전면 노출 | 상 | 완화 |
...
**요구사항** ⚠️ 기존 항목으로 완전히 덮이지 않음. 설계 문서에 프록시 구성 요건으로 넘겼으나 **별도 통제를 구성하거나 시험하지 않았다**(2026-09-26 확인). 남은 일은 `05-verification.md` 4절
...
#### `T-44` 의도적으로 취약한 검증 대상의 인터넷 전면 노출

**시나리오**
보호 대상 애플리케이션(OWASP Juice Shop)은 취약점을 학습하도록 **일부러 취약하게 만든 앱**이다. 경계 계층의 인바운드가 `0.0.0.0/0`이면 공인 주소가 할당되는 순간 인터넷 전역의 자동 스캐너에 노출되고, 룰셋이 없거나 탐지 전용인 단계(`ADR-018`)에서는 공격이 그대로 앱에 도달한다. 앱이 침해되면 NAT를 거친 아웃바운드로 **제3자를 공격하는 발판**이 될 수 있다(`T-43`).
이 위협은 요구사항 단계에서 누락되어 있었다. Phase 5에서 인바운드를 전면 개방할 때의 근거는 "경계 계층은 인터넷에서 도달해야 한다"였고, **누구로부터 도달해야 하는지는 정하지 않았다.** Phase 7 3단계 배포 중 접근 로그를 확인하며 발견했다.

**부수 피해** 외부 스캔 트래픽이 탐지율과 오탐 측정(`AC-05a`, `AC-05b`)의 로그에 섞여 **측정값을 오염시킨다.** 보안 위협이면서 검증 설계의 결함이다.

**가능성** 상 · **영향** 중 (침해 확산은 `SR-02`와 `SR-06`으로 제한되나 제3자 피해 가능) · **위험도** 상
**전략** 완화 · **통제** 경계 계층 인바운드 출발지를 운영자 주소로 한정한다. 주소는 코드에 기록하지 않고 실행 시점에 조회한다(`SR-10`). → `ADR-019`
**잔여 위험** 운영자가 공인 주소를 여러 사용자와 공유하는 망(통신사 NAT, 사내망)에 있으면 같은 주소의 제3자도 도달할 수 있다.
...
**이 저장소는 2026-09-04부터 공개돼 있어 특히 중요하다.** 공개 전에 커밋된 정보도 이력으로 함께 드러났으므로, 전환 전에 이력 전체에 계정 ID가 없는지 확인했다.
...
| 버전 | 일자 | 내용 |
|---|---|---|
| 1.4 | 2026-09-26 | 문서 정합성 정리 — 머리말 버전을 변경 이력과 맞춤(1.1로 남아 있었음), 공개 전환 완료 반영, `T-04`가 미검증으로 남은 사실 명시 |
| 1.3 | 2026-09-19 | **Phase 7 배포 중 발견 반영** — 경계 A에 `T-44`(취약 검증 대상의 인터넷 전면 노출) 추가. 인바운드 출발지를 정하지 않은 채 전면 개방했던 누락. 통제는 `ADR-019` |
