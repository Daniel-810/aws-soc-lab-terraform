...
### 관리형과 직접 운영 비교 (Phase 9)

같은 룰 파일을 같은 엔진 계열(Suricata)로 두 방식에 적용하고, 같은 요청을 보냈다.
두 방식 사이의 차이는 운영 모델에서 나온다([`ADR-022`](docs/adr/022-own-rules-for-both-approaches.md)).

| 항목 | 방식 A (관리형 방화벽) | 방식 B (Suricata on EC2) |
|---|---|---|
| 평문 경로에서 잡은 요청 | 17건 | 16건 |
| 암호화 경로 | 0건 | 0건 |
| 검사 계층 기동 | 5분 45초 | 약 1분 |
| 전체 삭제 | 약 5분 30초 | 78초 |
| 시간당 비용 (검사 계층) | 엔드포인트 $0.395 | 인스턴스 한 대와 공인 주소 하나 |
| 검사 프로세스가 멈추면 | 선택지 없음 (서비스가 처리) | 차단하도록 직접 구성, 시험으로 확인 |
| 이상 동작의 원인 | 내부 설정과 카운터를 볼 수 없음 | 설정과 카운터로 확인 가능 |

두 방식이 다르게 판정한 요청은 1건이다. 같은 교차 사이트 스크립팅 페이로드를 URL 경로에 넣으면 둘 다 탐지했고,
질의 문자열에 넣으면 관리형만 탐지했다. 인코딩된 질의 문자열을 디코딩하는 기본 설정의 차이로 추정한다.
나머지 16건은 같은 요청을 같은 룰 번호로 잡았다.

직접 운영 방식은 검사 프로세스가 멈추면 트래픽을 차단한다(fail-close). 멈추자 웹 서비스와 앱의 외부 통신이 끊겼고,
앱의 관리 접근도 함께 끊겼다. 앱의 관리 연결도 검사 계층을 지나기 때문이다. 재기동 뒤에는 재기동 전에 맺어진 연결이
버려져(Suricata 7의 예외 정책) 앱의 관리 접근이 약 6분 뒤에 돌아왔다. 결정과 확인 결과는
[`ADR-027`](docs/adr/027-suricata-inline-on-ec2.md)에 있다.
...
| 전체 생성 / 전체 삭제 | 방식 A 약 6분 / 약 5분 30초, 방식 B 약 2분 / 78초 | `NFR-02` (30분) |
| 검사 프로세스 중단 (방식 B) | 웹 방화벽 응답 끊김, 검사 인스턴스의 관리 접근 유지, 재기동 후 복구 | `ADR-012`, `ADR-027` |
...
| [`adr/`](docs/adr/) | 설계 결정 27건. 맥락 · 결정 · 근거 · 고려한 대안 · 결과 |
