...
> **Phase 7 현재 상태 — 웹 방화벽에 아직 규칙 집합이 적용되지 않았다.**
> 내부 TLS와 리버스 프록시까지 구성했고, 규칙 집합은 [`ADR-018`](docs/adr/018-crs-detect-measure-block.md)에 따라
> 기준선 측정, 탐지 전용 적용, 차단 전환의 순서로 적용한다. 최종 상태는 차단이다.
> 그 사이 앱이 노출되지 않도록 경계 계층 인바운드는 운영자 주소로 한정했다([`ADR-019`](docs/adr/019-waf-ingress-operator-only.md)).
...
| [`adr/`](docs/adr/) | 설계 결정 19건. 맥락 · 결정 · 근거 · 고려한 대안 · 결과 |
